...
gpointer systemWorkerLoop() {
    ...
    while (!appData.shutdownRequested && appData.isSensorWorkerRunning) {
        ...
        gdouble pressureValue = pressureReading->errorCount < SENSOR_WARNING_ERROR_COUNT ?
            pressureReading->value : pressureSensor->base.defaultValue;

        appData.system.isEngineRunning = appData.system.isIgnOn && pressureValue > pressureSensor->base.alertLow;
        ...
        buzzerOn = gpio_read(i2cPiHandle, BUZZER_GPIO_PIN) == TRUE;
        engineRpm = getEngineRpm();

        if (buzzerOn && (pressureValue >= pressureSensor->base.alertLow || engineRpm < OIL_PRESSURE_ALERT_MIN_RPM)) {
